...
export default function BookFormPage() {
    ...
    const fetchRelations = async () => {
        ...
        try {
            // FIX KRITIS: Menggunakan apiAuth untuk semua request Admin (Mengatasi 401/500)
            const [catRes, authRes, pubRes] = await Promise.all([
                apiAuth.get(CATEGORIES_ENDPOINT),
                apiAuth.get(AUTHORS_ENDPOINT),
                apiAuth.get(PUBLISHERS_ENDPOINT),
            ]);
            ...
            toast.error("Gagal memuat data relasi. Pastikan Anda sudah login sebagai Admin.");
        } finally {
